...
export class QuadraticBezier extends Shape {
    ...
    constructor(p0: { x: number; y: number }, p1: { x: number; y: number }, p2: { x: number; y: number }, transform?: Transform) {
        super({ transform });
        this.p0 = p0;
        this.p1 = p1;
        this.p2 = p2;
    ...
    getFlattenedPoints(): { x: number; y: number }[] {
        return flattenQuadraticLocal(this.p0, this.p1, this.p2, this.flatness);
    }
